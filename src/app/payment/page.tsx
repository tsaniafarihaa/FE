...
export default function PaymentPage() {
  ...
  const fetchTicketData = async () => {
    ...
    try {
      const [ticketResponse, couponResponse] = await Promise.all([
        fetch(`${process.env.NEXT_PUBLIC_BASE_URL_BE}/events/ticket/${ticketId}`),
        fetch(`${process.env.NEXT_PUBLIC_BASE_URL_BE}/orders/coupon-count/${ticketId}`, {
          headers: {
      ...
      const orderResponse = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL_BE}/orders`,
        {
          ...
          body: JSON.stringify({
            ...
            useCoupon: useCoupon && couponAvailable,
          }),
        }
      );
    ...
    <div className="min-h-screen bg-black text-white py-12">
      <div className="container mx-auto px-4">
        <div className="max-w-2xl mx-auto bg-zinc-900 rounded-xl p-8">
          <h1 className="text-3xl font-bold text-orange-500 mb-8">Payment Details</h1>

          <div className="space-y-6">
            <div className="bg-zinc-800 p-4 rounded-lg">
              <h2 className="text-xl font-semibold mb-4">{ticketData.event?.title}</h2>
              ...
              {user?.points && user.points >= 10000 && (
                <div className="flex items-center justify-between mb-3">
                  <div>
                    ...
                    <p className="text-sm text-gray-400">Available: {formatPrice(user.points)} pts</p>
                  </div>
            ...
            <div className="bg-zinc-800 p-4 rounded-lg">
              ...
              {usePoints && <p className="text-sm text-gray-400">-Rp 10,000 (Points)</p>}
              {useCoupon && (
                <p className="text-sm text-gray-400">
